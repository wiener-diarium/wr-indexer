...
for x in buchstaben:
    ...
    print("add Category")
    df["category"] = df.apply(lambda row: f"{x}", axis=1)
    ...
    for i, row in df.iterrows():
        ...
        if cur_code.startswith(f"{x}") and len(cur_code) == 6 and "9999" in cur_code:
            ...
            cur_code.startswith(f"{x}")
            and len(cur_code) == 5
            and cur_code.endswith("99")
        ):
            lookup[cur_code[:3]] = description

    g_df = pd.read_csv(g_file)
    g_df["full_code"] = g_df.apply(
        lambda row: f"{x}{row[f'{x}{x}{x}_kompakt']}", axis=1
    )

    print(f"merge {g_file} with {codes} and drop superfluous columns")
    simple = g_df.merge(df, how="left", on="full_code")[
        [f"ID_{x}", "full_code", "Beschreibung", "category"]
    ]
    ...
    new_df["top_concept"] = new_df.apply(
        lambda row: find_broader(row, lookup, 2), axis=1
    )
    print("add top broader")
    new_df["skos_broader"] = new_df.apply(
        lambda row: find_broader(row, lookup, 3), axis=1
    )
    dfs.append(new_df)
